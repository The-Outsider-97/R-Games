...
try:
    ...
    print(f"Failed to import ExecutionAgent: {e}")
    ExecutionAgent = None
...
try:
    ...
    print(f"Failed to import LanguageAgent: {e}")
    LanguageAgent = None

try:
    from src.agents.perception_agent import PerceptionAgent
except ImportError as e:
    print(f"Failed to import Perceptionpip install Agent: {e}")
    PerceptionAgent = None

try:
    from src.agents.learning_agent import LearningAgent
except ImportError as e:
    print(f"Failed to import LearningAgent: {e}")
    LearningAgent = None

try:
    from src.agents.planning_agent import PlanningAgent
except ImportError as e:
    print(f"Failed to import PlanningAgent: {e}")
    PlanningAgent = None

try:
    from src.agents.safety_agent import SafetyAgent
except ImportError as e:
    print(f"Failed to import SafetyAgent: {e}")
    SafetyAgent = None
...
class AgentFactory:
    ...
    def __init__(self, config: Optional[Dict[str, Any]] = None):
        ...
        for name, cls in self._agent_classes.items():
            # Get the correct module path based on the actual file structure
            if cls.__module__ == '__main__':
                # For classes defined in this file or dynamically
                module_path = f"ai.src.agents.{name}_agent"
            else:
    ...
    def discover_agents(self):
        import AI.src.agents  # root module
        for name, obj in inspect.getmembers(AI.src.agents):
