try:
    ...
    cv2 = None
    print("Warning: cv2 (OpenCV) not found. Rendering to image will be disabled.")
import copy
try:
    import torch
except ImportError:
    torch = None
    print("Warning: torch not found. SLAIEnv will not function correctly.")

try:
    import numpy as np
except ImportError:
    np = None
    print("Warning: numpy not found. SLAIEnv will not function correctly.")

try:
    import gymnasium as gym
except ImportError:
    gym = None
    print("Warning: gymnasium not found. SLAIEnv will not function correctly.")
...
class SLAIEnv(gym.Env):
    ...
    def _render_to_image(self):
        ...
        if cv2 is None:
            print("Warning: cv2 not available for rendering.")
            return None
