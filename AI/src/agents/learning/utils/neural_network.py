...
try:
    ...
    torch = None
    nn = None
    print("Warning: torch not found. Neural Network will not function.")

    # Dummy classes to prevent ImportErrors in class definitions
    class DummyModule:
...
try:
    ...
    class ReLU(Activation): pass
    class Sigmoid(Activation): pass
    class Tanh(Activation): pass
    class Linear(Activation): pass
...
class NeuralNetwork(nn.Module):
    ...
    def __init__(self, input_dim, output_dim):
        super().__init__()
        if torch is None:
            logger.warning("Torch not found. NeuralNetwork initialized in dummy mode.")
            return
