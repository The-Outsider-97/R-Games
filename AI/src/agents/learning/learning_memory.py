...
try:
    ...
    torch = None
    print("Warning: torch not found. LearningMemory will not function correctly.")
...
try:
    ...
    np = None
    print("Warning: numpy not found. LearningMemory will not function correctly.")
...
class LearningMemory:
    ...
    def sample_proportional(self, batch_size):
        ...
        with self.lock:
            ...
            if not np:
                # Fallback to uniform sampling if numpy is missing
                samples = self.sample(batch_size)
                return samples, [0]*len(samples), [1.0]*len(samples)
    ...
    def save_checkpoint(self, path=None):
        ...
        if not torch:
            logger.warning("Torch not found. Cannot save checkpoint.")
            return
    ...
    def load_checkpoint(self, path):
        ...
        if not torch:
            logger.warning("Torch not found. Cannot load checkpoint.")
            return
