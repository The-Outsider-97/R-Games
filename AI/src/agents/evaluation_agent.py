...
class EvaluationAgent(BaseAgent):
    def __init__(self, 
        ...
        self.model_dir = self.evaluation_config.get("model_dir", "AI/src/agents/evaluators/models")
...
if __name__ == "__main__":
    ...
    printer.status("Init", "Evaluation Agent initialized", "success")
    from src.agents.collaborative.shared_memory import SharedMemory
    from src.agents.agent_factory import AgentFactory
    shared_memory = SharedMemory()
    agent_factory = AgentFactory()

    agent = EvaluationAgent(shared_memory=shared_memory,agent_factory=agent_factory)
    print(agent)
    print(f"\n* * * * * Phase 2 - Logging * * * * *\n")
    results={}
    ...
    print(f"\n* * * * * Phase 5 - Training * * * * *\n")
    #df=pd.DataFrame({
    #    "timestamp": pd.date_range(start="2025-06-30 00:00:00", periods=100, freq="1s"),
    #    "severity": np.random.uniform(0, 1, size=100),
    #    "cognitive_complexity": np.random.normal(loc=3.0, scale=1.0, size=100),
    #    "data_flow_depth": np.random.randint(1, 5, size=100),
    #    "security_risk": np.random.uniform(0, 1, size=100),
    #    "agent_position_x": np.random.normal(loc=0.0, scale=1.0, size=100),
     #   "agent_position_y": np.random.normal(loc=0.0, scale=1.0, size=100),
    #    "sensor_temperature": np.random.normal(loc=35.0, scale=5.0, size=100),
    #    "system_load": np.random.uniform(0.2, 0.9, size=100),
    #    "network_latency_ms": np.random.normal(loc=80, scale=10, size=100),
    #    "collision_flag": np.random.choice([0, 1], size=100, p=[0.95, 0.05])
    #})
    #model1 = agent._train_anomaly_model()
    #detector = agent._train_deep_anomaly_detector(df=df)
    ...
    validation = suite.execute_full_validation(agent=agent, codebase=codebase_path)
